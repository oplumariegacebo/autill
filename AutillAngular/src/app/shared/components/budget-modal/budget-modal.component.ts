    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && Array.isArray(result.data)) {
        ...
        if (validItems.length > 0) {
          ...
          let ivaCalculator = (sumTotalPrice * (this.budgetForm.controls['Iva'].value / 100)).toFixed(2);
          let irpfCalculator = (sumTotalPrice * (this.budgetForm.controls['Irpf'].value / 100)).toFixed(2);

          this.budgetForm.controls['Price'].setValue(sumTotalPrice.toFixed(2) + ivaCalculator + irpfCalculator);
