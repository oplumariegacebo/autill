    ...
    this.budgetService.getBudgetById(id).subscribe((budget: any) => {
      this.userService.getUserById(budget.IdBusiness).subscribe((user: any) => {
        this.clientService.getClientById(budget.ClientId).subscribe((client: any) => {
          ...
          file.text(String(client.data.Name || ''), rightBlockX + labelWidth + 35, firstRowY, { align: 'left' });
          file.text('Email:', rightBlockX, firstRowY + rowHeight, { align: 'right' });
          file.text(String(client.data.Email || ''), rightBlockX + labelWidth + 35, firstRowY + rowHeight, { align: 'left' });
          file.text('NIF:', rightBlockX, firstRowY + rowHeight * 2, { align: 'right' });
          file.text(String(client.data.Nif || ''), rightBlockX + labelWidth + 35, firstRowY + rowHeight * 2, { align: 'left' });
          file.text('Dirección:', rightBlockX, firstRowY + rowHeight * 3, { align: 'right' });
          file.text(String(client.data.Address || ''), rightBlockX + labelWidth + 35, firstRowY + rowHeight * 3, { align: 'left' });
          file.text(String((client.data.Region || '') + ' ' + (client.data.Country || '')), rightBlockX + labelWidth - 35, firstRowY + rowHeight * 4, { align: 'left' });
          file.text('Teléfono:', rightBlockX, firstRowY + rowHeight * 5, { align: 'right' });
          file.text(String(client.data.PhoneNumber || ''), rightBlockX + labelWidth + 35, firstRowY + rowHeight * 5, { align: 'left' });
